...
function ThemeToggle({ isDark, onToggle }: { isDark: boolean; onToggle: () => void }) {
  return (
    <button
      ...
      className="p-2.5 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-amber-500 dark:text-cyan-400 border border-slate-300 dark:border-slate-700 transition-all duration-200 shadow-sm"
    >
...
/* -------------------------------------------------------------------------- */
/* REAL STORE LANDING PAGE (Root URL: /)                                      */
/* -------------------------------------------------------------------------- */
function StoreLandingPage({ isDark, toggleTheme }: { isDark: boolean; toggleTheme: () => void }) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      // Query real Supabase database first
      ...
      if (error || !data) {
        setSearchError(`Invoice "${searchId}" not found in database. Check invoice ID and try again.`);
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans selection:bg-cyan-500 selection:text-white transition-colors duration-300 overflow-x-hidden">
      ...
      {/* Glassmorphic Navbar */}
      <header className="border-b border-slate-200 dark:border-slate-800/60 bg-white/80 dark:bg-slate-950/70 backdrop-blur-2xl sticky top-0 z-50 px-4 md:px-8 py-4 shadow-sm">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-[10px] uppercase font-extrabold tracking-widest bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-500/30 px-2.5 py-0.5 rounded-full">
                  ERP Nexius
                </span>
              </div>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Smart Bill & Customer Self-Service</p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 md:px-8 py-10 md:py-16 space-y-16">
        ...
        <div className="text-center max-w-4xl mx-auto space-y-6">
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-indigo-500/10 border border-cyan-500/30 px-4 py-1.5 rounded-full text-xs font-bold text-cyan-600 dark:text-cyan-400 shadow-sm backdrop-blur-md">
          ...
          <p className="text-sm md:text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed">
            Received an instant WhatsApp or Email receipt link? Search your invoice below or sign in with your phone number.
          </p>
          ...
          <div className="max-w-2xl mx-auto pt-4">
            <form 
              ...
              className="flex flex-col sm:flex-row items-center bg-white dark:bg-slate-900/90 backdrop-blur-2xl border border-slate-300 dark:border-slate-800 p-2 sm:p-2.5 rounded-3xl shadow-xl space-y-2 sm:space-y-0 sm:space-x-2"
            >
              <div className="flex items-center w-full pl-3 text-slate-400">
                ...
                <input
                  ...
                  className="w-full bg-transparent border-none px-3 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none placeholder:text-slate-400 font-mono"
                />
              </div>
              <button
                type="submit"
                disabled={loading}
                className="w-full sm:w-auto px-7 py-3.5 bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 hover:opacity-95 text-white font-bold rounded-2xl text-xs flex items-center justify-center space-x-2 shrink-0 shadow-lg shadow-cyan-500/30 transition transform active:scale-95"
              >
            ...
            <div className="mt-3 flex items-center justify-center space-x-2">
              ...
              <button
                ...
                className="text-xs bg-slate-100 dark:bg-slate-800 text-cyan-600 dark:text-cyan-400 px-3 py-1 rounded-lg border border-slate-300 dark:border-slate-700 font-mono font-bold hover:underline"
              >
                INV-2026-8942
              </button>
            </div>
            ...
            {searchError && (
              <p className="text-xs text-rose-500 font-semibold mt-2">{searchError}</p>
            )}

            <div className="flex items-center justify-center space-x-4 text-xs text-slate-500 dark:text-slate-400 mt-3 font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6">
          <div className="group bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 p-8 rounded-3xl space-y-4 shadow-xl hover:border-cyan-500/40 transition-all duration-300 hover:-translate-y-1">
            <div className="w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center text-cyan-600 dark:text-cyan-400 shadow-inner group-hover:scale-110 transition-transform">
              <Receipt className="w-7 h-7" />
            </div>
            <h3 className="font-bold text-xl text-slate-900 dark:text-white">Instant Smart Receipts</h3>
          ...
          <div className="group bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 p-8 rounded-3xl space-y-4 shadow-xl hover:border-emerald-500/40 transition-all duration-300 hover:-translate-y-1">
            <div className="w-14 h-14 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-inner group-hover:scale-110 transition-transform">
              <ShieldCheck className="w-7 h-7" />
            </div>
            <h3 className="font-bold text-xl text-slate-900 dark:text-white">Automated Warranty Vault</h3>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              Every device serial number and IMEI is automatically backed up in our cloud database for effortless warranty claims.
            </p>
          </div>

          <div className="group bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 p-8 rounded-3xl space-y-4 shadow-xl hover:border-amber-500/40 transition-all duration-300 hover:-translate-y-1">
            <div className="w-14 h-14 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-600 dark:text-amber-400 shadow-inner group-hover:scale-110 transition-transform">
              <Wrench className="w-7 h-7" />
            </div>
            <h3 className="font-bold text-xl text-slate-900 dark:text-white">Real-Time Repair Claims</h3>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
        ...
        <div className="bg-white dark:bg-slate-900/90 backdrop-blur-2xl border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-2xl max-w-xl mx-auto space-y-6">
          <div className="text-center space-y-1.5">
            ...
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Access Customer Portal</h2>
          ...
          {!userLoggedIn ? (
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-2xl px-4 py-3.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-cyan-500 font-mono"
                />
              </div>
            ...
            <div className="text-center space-y-4 bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800">
      ...
      {/* Footer */}
      <footer className="border-t border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/60 text-center py-8 text-xs text-slate-500 space-y-3">
        ...
        <div className="flex justify-center items-center space-x-6 text-[11px] font-semibold">
          <Link to="/demo-hub" className="text-cyan-600 dark:text-cyan-400 hover:underline flex items-center space-x-1">
            <span>ERP System Delivery Templates</span>
  ...
  if (!invoice) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 text-center space-y-4">
        ...
        <p className="text-xs text-slate-400">The requested invoice ID "{id}" does not exist in the database.</p>
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 font-sans selection:bg-cyan-500 selection:text-white transition-colors duration-300">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="no-print bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-3xl p-4 md:p-5 flex flex-wrap items-center justify-between gap-4 shadow-xl">
          <div className="flex items-center space-x-3">
            <div className="bg-emerald-500/10 border border-emerald-500/30 p-2.5 rounded-2xl text-emerald-500">
              <CheckCircle2 className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-sm font-bold text-slate-900 dark:text-white">I-STORE Smart Receipt</span>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Sent via WhatsApp & Email to {activeInvoice.customerPhone}</p>
            </div>
          </div>
        ...
        <div id="printable-invoice" className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-10 shadow-2xl space-y-8 relative overflow-hidden transition-colors">
          
          {/* Header & QR Code */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 dark:border-slate-800 pb-8 gap-6">
            <div>
              <div className="flex items-center space-x-2">
                <Sparkles className="w-6 h-6 text-cyan-500" />
                <h1 className="text-2xl font-black tracking-wider text-slate-900 dark:text-white">I-STORE MOBILE</h1>
              </div>
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Liberty Plaza, Colombo 03 | Support: +94 11 234 5678</p>
              <p className="text-xs text-slate-400 dark:text-slate-500">Tax ID: 90218-VAT</p>
            </div>

            {/* Smart QR Code Receipt */}
            <div className="flex items-center space-x-4 bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 p-3 rounded-2xl">
              ...
              <div className="text-left text-xs">
                ...
                <span className="text-[10px] text-slate-500 dark:text-slate-400 block mt-0.5">Verification QR</span>
                <span className="font-mono text-[11px] text-slate-700 dark:text-slate-300 font-bold block">{activeInvoice.id}</span>
              </div>
            </div>
          </div>

          {/* Customer Metadata */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 text-xs">
            <div>
              <span className="text-slate-500 dark:text-slate-500 uppercase font-semibold block mb-0.5">Customer</span>
              <span className="font-bold text-slate-900 dark:text-slate-200">{activeInvoice.customerName}</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-500 uppercase font-semibold block mb-0.5">Phone Number</span>
              <span className="font-medium text-slate-700 dark:text-slate-300">{activeInvoice.customerPhone}</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-500 uppercase font-semibold block mb-0.5">Payment</span>
              <span className="font-medium text-emerald-500">{activeInvoice.paymentMethod}</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-500 uppercase font-semibold block mb-0.5">Earned Points</span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400 uppercase font-semibold">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800/50">
                {activeInvoice.items.map((item: InvoiceItem, idx: number) => (
                  <tr key={idx}>
                    ...
                    <td className="py-3.5 px-2 text-center text-slate-700 dark:text-slate-300 font-medium">{item.qty}</td>
                    <td className="py-3.5 px-2 text-right text-slate-700 dark:text-slate-300">{item.price.toLocaleString()}</td>
                    <td className="py-3.5 px-2 text-right font-bold text-slate-900 dark:text-slate-100">
          ...
          {/* Total */}
          <div className="border-t border-slate-200 dark:border-slate-800 pt-4 flex flex-col items-end text-sm space-y-1">
            <div className="w-full md:w-64 flex justify-between text-slate-600 dark:text-slate-400">
              <span>Subtotal</span>
            ...
            <div className="w-full md:w-64 flex justify-between text-xl font-black text-cyan-500 border-t border-slate-200 dark:border-slate-800 pt-2">
          ...
          {/* Quick Actions */}
          <div className="no-print bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 text-xs">
            <div className="flex items-center space-x-3">
              <Wrench className="w-5 h-5 text-amber-500" />
              <div>
                <p className="font-bold text-slate-900 dark:text-slate-200">Need Service or Repair?</p>
                <p className="text-slate-600 dark:text-slate-400">Request warranty claims or repairs directly online.</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <Link
                to="/"
                className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-semibold rounded-xl border border-slate-300 dark:border-slate-700 transition flex items-center space-x-1"
              >
      ...
      {repairModalOpen && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400">Device: iPhone 15 Pro Max (IMEI: 359102910293819)</p>
